...
if st.session_state.step == "select_theme":
    ...
    user_text = st.text_input("✏️ 한 줄 추억 남기기 (최대 20자)", placeholder="예: 인사동 거리에서 우리의 첫 필드 테스트!")
...
# --- STEP 2: 렌더링 (모의 로딩) ---
    ...
    images = []
    # 5장 이미지 리사이징 및 텍스트 합성 처리 (버그 없는 런타임)
    for i, file in enumerate(st.session_state.uploaded_files):
        try:
            ...
            draw.text((10, 360), f"{st.session_state.user_text}", fill="white")
            draw.text((10, 380), f"{datetime.now().strftime('%Y.%m.%d %H:%M')} | Seoul", fill="white")
